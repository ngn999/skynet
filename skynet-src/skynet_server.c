...
skynet_context_new(const char * name, const char *param) {
	struct skynet_module * mod = skynet_module_query(name); /* lazy¼ÓÔØ */

	if (mod == NULL)                                        /* not found */
		return NULL;

    /* modeµÄcreateº¯Êý£¬»á´´½¨Ò»Ð©¸ÃÄ£¿éÒªÓÃµ½µÄÊý¾Ý;
     * Ã¿¸öÊµÀýµÃÓÐ×Ô¼ºµÄÊý¾Ý */
...
skynet_context_message_dispatch(struct skynet_monitor *sm) {
	...
	if (ctx->cb == NULL) {
        ...
        /* ¸ù¾Ý`msg'µÄtypeÀ´¾ö¶¨ÊÇcall ctx->cb£¬»¹ÊÇ×öfwd */
